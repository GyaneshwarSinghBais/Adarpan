...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, FlatList, RefreshControl, ScrollView } from 'react-native';
import { fetchCGMSCStockQTY, fetchMainCategoryService, fetchPublicStockitems, fetchRCDetail, fetchWarehouseStockReport, fetchdhsDmeYearConsumption, fetchgetDhsDmeStock, fetchitemIndentQuantity } from './Services/apiService';
...
const TableComponent = ({ navigation }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      setVisible(true);
      //alert("value:" + value);
      const response = await fetchCGMSCStockQTY(value, "0",id,roleid);
      setData(response);
    } catch (error) {
  ...
  const fetchDataWithMCAt = async () => {
    ...
    try {
      ...
      {
        SetTitalValue("CGMSC Stock Detail");
        setWHCGMSCValue("CGMSC");
      }



      //alert("userid" + facid);
      const response = await fetchCGMSCStockQTY(value, "0", facuserid,"0","0");
      setData(response);
      if (facid == null) {
        const responseRCDetail = await fetchRCDetail(value);
        setDataRC(responseRCDetail);
      }
      const responseIndentQuantity = await fetchitemIndentQuantity(value, facuserid, "0","0","0");
      setDataIndentQty(responseIndentQuantity);
      const responseDhsDmeStock = await fetchgetDhsDmeStock(value, "0", facuserid, "0", "0","0","0");
      setgetDhsDmeStock(responseDhsDmeStock);
      const responseDmeYearConsumption = await fetchdhsDmeYearConsumption(value, "0", facuserid, "0", "0","0","0");
    ...
    return (
      ...
      <View style={styles.row}>
        <View style={styles.column}>
          ...
          <View style={styles.rowItem}>
            <Title style={styles.label}>{WHCGMSCValue} Stock:</Title>
            <Paragraph style={styles.value}>{item.readyforissue}</Paragraph>
          </View>

          <View style={styles.rowItem}>
            <Title style={styles.label}>{WHCGMSCValue} UQC Stock:</Title>
            <Paragraph style={styles.value}>{item.pending}</Paragraph>
          </View>
          <View style={styles.rowItem}>
            <Title style={styles.label}>{WHCGMSCValue} Pipeline Stock:</Title>
    ...
    return (
      ...
      <View style={styles.row}>
        <View style={styles.column}>
          ...
          <View style={styles.rowItem}>
            <Title style={styles.label}>DHS Annual Indent for {WHCGMSCValue}:</Title>
            <Paragraph style={styles.value}>{item.dhsai}</Paragraph>
          </View>
          <View style={styles.rowItem}>
            <Title style={styles.label}>DHS Issued Quantity by {WHCGMSCValue}:</Title>
            <Paragraph style={styles.value}>{item.dhsissue}</Paragraph>
          </View>

          <View style={styles.rowItem}>
            <Title style={styles.label}>DHS Issued% by {WHCGMSCValue}:</Title>
            <Paragraph style={styles.value}>{item.dhsissuep}</Paragraph>
          </View>

          <View style={styles.rowItem}>
            <Title style={styles.label}>DME Annual Indented Items for {WHCGMSCValue}:</Title>
            <Paragraph style={styles.value}>{item.dmeai}</Paragraph>
          </View>

          <View style={styles.rowItem}>
            <Title style={styles.label}>DME Issued Quantity by {WHCGMSCValue}</Title>
            <Paragraph style={styles.value}>{item.dmeissue}</Paragraph>
          </View>

          <View style={styles.rowItem}>
            <Title style={styles.label}>DME Issued% by {WHCGMSCValue}:</Title>
    ...
    return (
      ...
      <View style={styles.row}>
        <View style={styles.column}>
          ...
          <Title style={styles.header}>Valid Rate Contract</Title>

          <View style={styles.rowItem}>
            <Title style={styles.label}>Rate(in Tax):</Title>
            <Paragraph style={styles.value}>{item.rcRate}</Paragraph>
          </View>

          <View style={styles.rowItem}>
            <Title style={styles.label}>Start Dt:</Title>
            <Paragraph style={styles.value}>{item.rcStartDT}</Paragraph>
          </View>
          <View style={styles.rowItem}>
            <Title style={styles.label}>End Dt:</Title>
          ...
          <View style={styles.rowItem}>
            ...
            <Paragraph style={styles.value}>{item.schemecode}</Paragraph>
          </View>
        </View>
      </View>
    ...
    return (
      ...
      <View style={styles.row}>
        <View style={styles.column}>
          ...
          <View style={styles.rowItem}>
            <Title style={styles.label}>Stock:</Title>
          ...
          <View style={styles.rowItem}>
            <Title style={styles.label}>Stock SKU:</Title>
            <Paragraph style={styles.value}>{item.fieldstocksku}</Paragraph>
          </View>
  ...
  const navigateFunctionFile = (item) => {
    ...
    navigation.navigate('District Wise DHS & DME Stock', { item: item, });
  }
    ...
    return (
      ...
      <View style={styles.row}>
        <View style={styles.column}>
          ...
          <View style={styles.rowItem}>
            <Title style={styles.label}>Issued to OPD/IPD (SKU):</Title>
            <Paragraph style={styles.value}>{item.issuedsku}</Paragraph>
          </View>
