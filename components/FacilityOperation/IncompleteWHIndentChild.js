...
const IncompleteWHIndentChild = ({ navigation, props }) => {
  ...
  const fetchFacMonthIndentItemsData = async () => {
    try {
      //alert("fetchFacMonthIndentItemsData childPage id: "+ id +"value: "+ value + "INDENTID: " + INDENTID);
      ...
      console.error('Error Gyan:', error);
    }
  };

  const fetchfacIndentAlert1Data = async () => {
    try {
      //alert("values :" +id+ value+ 'NA'+ value1)
    ...
    <View style={styles.cardContent}>
            ...
            {data.length > 0 ? (
              <DropDownPicker
                ...
                onChangeValue={(value) => {
                  ...
                  // if (value !== null || value.trim() !== '' || value !== undefined || value.trim().length > 0) {
                    if(!isFirstOpen){
                    setOldValue(value);

                    if (value != oldValue) {
                      //alert("fistdropdown selected index value: " + value + "oldvalue: " + oldValue);
          ...
          {indentAlertData.length > 0 ? (
            <View style={styles.card}>
              ...
              <View style={styles.cardContent}>
                <View style={StyleSheet.flatten([styles.cardItemRow, { justifyContent: 'space-between', flexDirection: 'row' }])}>
                  <Text style={styles.labelRow}>Fac Stock: </Text>
                  {/* <Text >{JSON.stringify(indentAlertData)}</Text> */}
                  <Text>{indentAlertData[0].facstock}</Text>
                  <Text>  |  </Text>
                  <Text style={styles.labelRow}>WH Stock: </Text>
                  <Text >{indentAlertData[0].whready}</Text>
                  <Text>  |   </Text>
                  <Text style={styles.labelRow}>Unit Count:</Text>
                  <Text >{indentAlertData[0].unitcount}</Text>
                </View>
                <View style={StyleSheet.flatten([styles.cardItemRow, { justifyContent: 'space-between', flexDirection: 'row' }])}>
